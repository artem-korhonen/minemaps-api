...
def get_map_service(repository: MapRepository = Depends(get_map_repository)) -> MapService:
...
def get_current_user_id_optional(token: str | None = Depends(oauth2_scheme)) -> int | None:
    if token is None:
        return None
...
async def get_current_user(current_user_id: int = Depends(get_current_user_id), service: UserService = Depends(get_user_service)) -> User:
...
async def get_current_user_optional(current_user_id: int | None = Depends(get_current_user_id_optional), service: UserService = Depends(get_user_service)) -> User | None:
