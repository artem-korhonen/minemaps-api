...
class User(Base):
    ...
    role: Mapped[UserRole] = mapped_column(String(16), server_default=UserRole.USER, nullable=False)
    profile_image_url: Mapped[str | None] = mapped_column(String(128), server_default=None, nullable=True)
...
class FollowUser(Base):
    ...
    followed_user_id: Mapped[int] = mapped_column(ForeignKey("users.id"), primary_key=True)
